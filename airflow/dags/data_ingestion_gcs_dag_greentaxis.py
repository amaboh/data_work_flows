...
def format_to_parquet(src_file):
    if not src_file.endswith('.csv'):
        logging.error("Sorry we can only accept source files in CSV format at the moment")
        return
...
def upload_to_gcs(bucket, object_name, local_file):
    ...
    storage.blob._DEFAULT_CHUNK_SIZE = 5 * 1024 * 1024 # 5 MB   
    # End of workaround
